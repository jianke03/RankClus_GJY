...
def rank(i,author_score_cluster,confer_score_cluster,confer_score_in_cluster,clu):
    print('in ranking cluster ' + str(i))
    author_score_cluster[i], confer_score_cluster[i], confer_score_in_cluster[i] = authorityRank(author_confer, confer_author, author_author, clu, rankT,alpha)
...
while rankclus_iter < T:
    ...
    for i in range(K):
        pool.apply_async(rank, (i,author_score_cluster,confer_score_cluster,confer_score_in_cluster,cluster[i]))
    pool.close()
    pool.join()
